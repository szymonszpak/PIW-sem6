...
export default function Home() {
  ...
  const fetchGames = async () => {
    try {
      ...
      console.error("Błąd pobierania z Firestore:", error);
    }
  };
  ...
  const handleBuy = async (gameId) => {
    ...
    try {
      await updateDoc(doc(db, "games", gameId), { isSold: true });
      fetchGames();
...
const handleSubmit = async (e) => {
  ...
  const handleDelete = async () => {
    if (confirm("Na pewno chcesz usunąć tę grę?")) {
      await deleteDoc(doc(db, "games", formData.id.toString()));
      fetchGames();
      setIsModalOpen(false);
    }
  };
          ...
          <section className="products-grid">
            {paginatedGames.map((game) => (
              <article key={game.id} className="product-card" 
                ...
                {game.isSold && (
                  <div style={{ position: 'absolute', top: '40%', left: '50%', transform: 'translate(-50%, -50%)', background: 'red', color: 'white', padding: '10px 20px', fontWeight: 'bold', borderRadius: '5px', zIndex: 5, fontSize: '20px' }}>
                    SPRZEDANE
                  </div>
                )}
